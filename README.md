...
| **J** | (밭) 지난 밭일 일지 열람 |
| **ESC** | 설정 열기 (메뉴 화면에서는 뒤로가기, 일지 팝업은 닫기) |
| **M** | 음소거 토글 |
| **F11** | 전체 화면 토글 (설정창에서도 가능) |
| **F12** | 스크린샷 저장 (세이브 폴더 `screenshots/`) |
| **R** | (엔딩 화면) 다시하기 |

텍스트 속도(느림/보통/빠름)는 **설정**에서 바꿀 수 있습니다.
...
- **태도로 갈리는 엔딩** — 점수가 아니라 인내·공감·이해가 결말(진·노멀·배드·시듦, 그리고
  하나 더)을 정하고, 결과 화면이 '왜 이 엔딩인지'를 알려줍니다.
- **여러 작물, 서로 다른 이야기** — 엔딩을 한 번 보면 사과나무·감자·벼가 열립니다. 작물마다
  밭의 성질·미니게임·수확 방식은 물론 **고유 회상과 사건, 엔딩의 마지막 문장**까지 다릅니다.
- **매번 다른 회차** — 회차마다 '해의 성격'(가뭄해·장마해·풍년해…)이 날씨의 기질을 정하고,
  16가지 선택 사건이 지난 회차와 겹치지 않게 찾아옵니다.
- **아버지의 창고** — 회차의 성과로 아버지의 물건 9가지를 하나씩 되찾는 컬렉션. 완주한
  회차의 일지는 영구 보관되어 언제든 다시 읽을 수 있고, 누적 통계가 쌓입니다.
- **업적 24종 · 갤러리** — 잠금 해제식 업적과 엔딩·사건·회상 갤러리 (스팀 도전과제 연동 준비 완료).
- **그 너머** — 진엔딩을 보면 열리는 **악)몽중농원**(지옥 모드)과 **또 하나의 새벽**,
  엔딩 3종을 모으면 열리는 도전 규칙(무일지·한발·이레).
- **한/영 현지화** — 렌더 계층 자동 번역으로 UI·서사까지 전환. 번역 기여는 GitHub PR로 받습니다([`LOCALIZATION.md`](LOCALIZATION.md)).
...
core/
  ...
  i18n.py / i18n_data.py / locales/   현지화 엔진 · 카탈로그 · 번역 JSON(언어별)
  palette.py / pixelfx.py   공용 색 팔레트 · 큰 픽셀 프리미티브(챔퍼·도트 글로우)
  assets.py       픽셀 스프라이트 · 폰트 · 작물/아이콘 그래픽 · 배경
  audio.py        절차적 오디오(런타임 합성 / 구운 OGG)
  layout.py       적응형 캔버스(창 비율별 안전영역·여백)
  storehouse.py   아버지의 창고 — 물건 컬렉션 정의·해금 판정
...
scenes/           title · intro · crop_select · farm · stage1~4 · tending · story_choice ·
                  memory · epiphany · star_connect · father_day · gallery · ending ·
                  transition · epilogue
```
...
| [`LOCALIZATION.md`](LOCALIZATION.md) | 현지화 시스템 · GitHub PR 번역 워크플로 · 문구 추가법 |
| [`CONTRIBUTING.md`](CONTRIBUTING.md) | 기여 안내(번역 중심) · 라이선스 기여 조건 |
| [`STEAM.md`](STEAM.md) | 스팀 출시 준비 상태 · Auto-Cloud/데모 빌드 절차 |
| [`IDEAS_STEAM.md`](IDEAS_STEAM.md) | 플레이타임·흥미 확장 로드맵과 진행 현황 |
| [`DESIGN_AUDIT.md`](DESIGN_AUDIT.md) | UI/디자인 일관성 감사 · P1~P4 로드맵 |
| [`RELEASING.md`](RELEASING.md) | 안드로이드 정식 출시(서명 AAB) 가이드 |
| [`THIRD-PARTY-NOTICES.md`](THIRD-PARTY-NOTICES.md) | 번들 폰트·라이브러리 제3자 라이선스 |
...
이 게임의 소스 코드와 리소스(그래픽·사운드·텍스트·스토리 등)는 **삼광(三光)** 팀의 자산이며 저작권법의
보호를 받습니다. 소스는 학습·검토를 위해 공개되어 있을 뿐 **오픈소스 라이선스가 부여된 것은 아니며**,
무단 복제·배포·상업적 이용을 금합니다. 전체 조건은 [`LICENSE`](LICENSE)를 참고하세요.

- **기여(번역 등)** — PR로 제출한 기여물은 게임에 상업 배포 포함 사용할 수 있도록 라이선스를 부여하는
  조건으로 접수됩니다. 방법은 [`CONTRIBUTING.md`](CONTRIBUTING.md)·[`LOCALIZATION.md`](LOCALIZATION.md).
- **제3자 라이선스** — 번들 폰트 **갈무리11(Galmuri11)**(SIL OFL 1.1)·pygame-ce(LGPL) 등은
