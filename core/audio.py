...
def _bgm_event():
    ...
    8화음(약 15초)이라 반복감이 강했다 → 16화음(약 35초)으로 늘려 반복을 줄인다."""
    ...
    # 16개 화음 진행: 두 바퀴 돌면서 변주가 들어가 반복감을 줄인다
    pad = _pad_progression([
        Am, Dm, F, E,      # 1절
        Am, Gm, Dm, E,     # 2절 (Gm 추가)
        F, Dm, Bb, Am,     # 3절 (Bb로 변주)
        Gm, C, Dm, E,      # 4절 (C 추가)
    ], 2.2, xfade=0.22)
    ...
    # 불안하게 맴도는 아르페지오 — 늘어난 pad 길이에 맞춰 더 오래 이어지게 한다.
    arp = _melody([(440.0, 1), (523.25, 1), (659.25, 1), (587.33, 1)] * 36, 0.225)
