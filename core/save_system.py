...
def reset_all():
    ...
    행동 데이터(run_*.jsonl·sessions.jsonl·pending_upload.json)도 함께 지운다 —
    남으면 '완전 초기화' 이후에도 lifetime_scan/갤러리/리포트가 옛 회차로 오염된다."""
    global _meta_cache
    ok = False
    ...
    shutil.rmtree(os.path.join(_DIR, "behavior"), ignore_errors=True)
    return ok
