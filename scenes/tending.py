...
import random
import pygame
from core.assets import (sprites, get_font, GOLD, SUN, CLOUD, RAIN,
                         WOOD_COLOR, WOOD_DARK, WOOD_LIGHT)
from core.pixelfx import pixel_rect, pixel_disc, pixelate, CHAMFER, CHAMFER_SM
...
class WaterPour(MiniGameBase):
    ...
    def __init__(self, farm):
        ...
        # 후반 변형 — 성장 60%를 넘으면 뿌리가 예민해져 '적정' 구간이 눈에 띄게 좁아진다
        frac = farm.sim.growth / max(1, farm.sim.growth_goal)
        if frac >= 0.6:
    ...
    def draw(self, screen):
        ...
        gbot = gtop + gh
        pixel_rect(screen, (40, 34, 28), (gx - 2, gtop - 2, gw + 4, gh + 4), chamfer=CHAMFER_SM)
        pixel_rect(screen, (24, 30, 36), (gx, gtop, gw, gh), chamfer=CHAMFER_SM)
        ...
        pixel_rect(screen, (90, 170, 220), (gx, cur_y, gw, gbot - cur_y), chamfer=CHAMFER_SM)
        pygame.draw.rect(screen, (210, 235, 250), (gx, cur_y, gw, 3))
        pixel_rect(screen, (90, 78, 60), (gx, gtop, gw, gh), width=2, chamfer=CHAMFER_SM)
    ...
    def draw(self, screen):
        ...
        for w in self.items:
            ...
            if is_apple:
                ...
                pixel_disc(screen, leaf_col1, (gx2 - 6, gy2 - 12), 7, px=2)
                pixel_disc(screen, leaf_col2, (gx2 + 9, gy2 - 9), 6, px=2)
                pixel_disc(screen, leaf_col3, (gx2 + 2, gy2 - 15), 5, px=2)
                # 잡는 곳 표시
                glow = pygame.Surface((30, 30), pygame.SRCALPHA)
                pygame.draw.circle(glow, (240, 255, 210, 90), (15, 15), 14)
                screen.blit(pixelate(glow, 3, smooth=False), (gx2 - 15, gy2 - 15))
            else:
                # 대상 표시
                ...
                screen.blit(pixelate(glow, 3, smooth=False), (gx2 - 23, gy2 + 6))
...
class PestTap(MiniGameBase):
    ...
    def __init__(self, farm):
        ...
        # 강풍 변형 — 벌레가 바람에 흔들려 더 빠르고 변덕스럽게 움직인다 (굼벵이는 땅속이라 무풍)
        self.windy = (game_state.weather == "강풍" and game_state.crop != "potato")
        if self.windy:
            for b in self.bugs:
                b["vx"] *= 1.35
                b["vy"] *= 1.35
        ...
        if game_state.crop == "potato":
            self.PROMPT = "밭의 굼벵이를 빠르게 잡으세요"
        elif self.windy:
            self.PROMPT = "강풍에 벌레가 이리저리 날린다! 빠르게 잡으세요"
        else:
            self.PROMPT = "잎의 벌레를 빠르게 잡으세요"
    ...
    def _update_game(self, dt):
        ...
        for b in self.bugs:
            if b["dead"]:
                continue
            # 강풍 변형 — 이따금 돌풍에 방향이 홱 꺾인다
            if getattr(self, "windy", False) and random.random() < dt * 2.2:
                ang = random.uniform(-1.1, 1.1)
                v = pygame.math.Vector2(b["vx"], b["vy"]).rotate_rad(ang)
                b["vx"], b["vy"] = v.x, v.y
            b["x"] += b["vx"] * dt
            b["y"] += b["vy"] * dt
...
def _pill(screen, rect, alpha=190):
    ...
    pixel_rect(bg, (16, 18, 24, alpha), bg.get_rect(), chamfer=CHAMFER)
    pixel_rect(bg, (96, 84, 64, 160), bg.get_rect(), width=1, chamfer=CHAMFER)
...
def _caption(screen, text, color=None, hot=False):
    ...
    # 긴 문장(특히 영어)이 화면 밖으로 나가지 않게: 폰트 축소 + 화면 안으로 클램프
    while t.get_width() > 776 and size > 12:
        size -= 1
        f = _font(size)
        t = f.render(text, True, color)
...
def _counter(screen, text, time_frac):
    ...
    screen.blit(t, (x, y))
    # 남은 시간 바 — 왼쪽 카운터와 같은 알약 백드롭을 깔아 프레임 위에 '스티커'처럼 뜨지 않게,
    # 채움은 팔레트 GOLD 로 통일
    w = 120
    bar = pygame.Rect(PLOT.right - w - 8, PLOT.y + 8, w, 6)
    _pill(screen, bar.inflate(14, 10), alpha=170)
    pixel_rect(screen, (40, 34, 28), bar, chamfer=CHAMFER_SM)
    fill_w = int(w * max(0.0, min(1.0, time_frac)))
    if fill_w > 0:
        pixel_rect(screen, GOLD, (bar.x, bar.y, fill_w, bar.h), chamfer=CHAMFER_SM)
...
class WeatherSunshine:
    ...
    def draw(self, screen):
        _veil(screen)
        # 오브젝트는 밭 안으로 클리핑 — 위에서 내려올 때 상단 배너/프레임을 덮지 않는다
        ...
        for orb in self.orbs:
            ...
            g = glow_sprite(r * 2, SUN, px=4, steps=(55, 130), core=((255, 246, 190), 210))
            blit_glow(screen, g, (x, y))
        for s in self.sparkles:
            r = int(12 * (1 - s[2] / 0.4)) + 3
            surf = pygame.Surface((r * 2, r * 2), pygame.SRCALPHA)
            pygame.draw.circle(surf, (255, 255, 200, int(200 * (s[2] / 0.4))), (r, r), r)
            screen.blit(pixelate(surf, smooth=False), (int(s[0]) - r, int(s[1]) - r))
...
class WeatherRain:
    ...
    def draw(self, screen):
        ...
        old_clip = screen.get_clip()
        screen.set_clip(PLOT)
        # 빗방울 — 2px 슬래시는 암막 위에서 안 보였다 → 3px 블록 기둥(잔상+코어)으로 키움
        for drop in self.drops:
            x, y = int(drop["x"]), int(drop["y"])
            pygame.draw.rect(screen, (RAIN[0], RAIN[1], RAIN[2]), (x - 1, y - 6, 3, 6))   # 잔상(짙은)
            pygame.draw.rect(screen, (176, 208, 250), (x - 1, y, 3, 9))                   # 코어(밝은)
        # 양동이 — 사선 폴리곤 대신 WOOD 팔레트의 픽셀 블록 계단(곡선/사선 없음)
        bx, by = int(self.bucket_x), int(self.bucket_y)
        pygame.draw.rect(screen, WOOD_LIGHT, (bx - 18, by - 16, 36, 3))   # 림 하이라이트
        pygame.draw.rect(screen, WOOD_DARK, (bx - 18, by - 13, 36, 4))    # 림 테
        pygame.draw.rect(screen, WOOD_COLOR, (bx - 15, by - 9, 30, 9))    # 몸통 상단
        pygame.draw.rect(screen, WOOD_COLOR, (bx - 12, by, 24, 8))        # 몸통 하단(계단)
        pygame.draw.rect(screen, WOOD_DARK, (bx - 12, by + 5, 24, 3))     # 바닥 그늘
        pygame.draw.rect(screen, WOOD_DARK, (bx - 6, by - 9, 2, 17))      # 판자 이음새
        pygame.draw.rect(screen, WOOD_DARK, (bx + 4, by - 9, 2, 17))
        # 물 튀김
        for s in self.splashes:
            r = int(8 * (1 - s[2] / 0.3)) + 2
            surf = pygame.Surface((r * 2, r * 2), pygame.SRCALPHA)
...
class WeatherCloudy:
    ...
    def __init__(self, farm):
        ...
        for _ in range(self.target):
            ...
            # 구름 스프라이트를 초기화 때 1회 도트화 프리렌더 (매 프레임 타원 그리기 대신)
            surf = pygame.Surface((w, h), pygame.SRCALPHA)
            pygame.draw.ellipse(surf, (CLOUD[0], CLOUD[1], CLOUD[2], 200), (0, h // 4, w, h * 3 // 4))
            pygame.draw.ellipse(surf, (224, 228, 238, 220), (w // 5, 0, w * 3 // 5, h * 3 // 4))
            self.clouds.append({
                "x": random.randint(PLOT.x + 20, PLOT.right - 80),
                "y": random.randint(PLOT.y + 10, PLOT.y + 120),
                "w": w,
                "h": h,
                "sprite": pixelate(surf, smooth=False),   # 큰 픽셀: 구름 도트화
                "alive": True,
...
class WeatherDrought:
    ...
    def __init__(self, farm):
        ...
        # 갈라진 땅 — 매판 다른 자리에, 헤어라인 대신 4px 블록 계단 세그먼트로
        self.cracks = []
        for _ in range(7):
            x = random.randint(PLOT.x + 24, PLOT.right - 44)
            y = random.randint(PLOT.y + 30, PLOT.bottom - 90)
            segs = []
            for _ in range(random.randint(3, 5)):
                dy = random.randint(8, 14)
                segs.append((x, y, 4, dy))
                x += random.choice([-4, 4])
                y += dy
    ...
    def draw(self, screen):
        _veil(screen)
        # 갈라진 땅 — 마른 흙 가장자리(밝은 톤) + 틈(어두운 톤) 2톤 블록 계단.
        # 어두운 단색만으로는 암막 낀 흙 텍스처에 묻혀 보이지 않았다.
        for segs in self.cracks:
            for (sx, sy, sw, sh) in segs:
                pygame.draw.rect(screen, (128, 98, 66), (sx - 2, sy, sw + 4, sh))
                pygame.draw.rect(screen, (44, 30, 22), (sx, sy, sw, sh))
        for spot in self.spots:
            if spot["found"]:
                # 찾은 수맥 — 물 표시 (screen 직접 draw 는 알파가 무시되므로 SRCALPHA 서피스에 그려 도트화)
                mark = pygame.Surface((28, 28), pygame.SRCALPHA)
                pygame.draw.circle(mark, (100, 180, 255, 150), (14, 14), 12)
                pygame.draw.circle(mark, (150, 210, 255, 255), (14, 14), 6)
                screen.blit(pixelate(mark, smooth=False), (int(spot["x"]) - 14, int(spot["y"]) - 14))
            elif spot["active"]:
                # 빛나는 수맥 — 깜빡깜빡
                ...
                screen.blit(pixelate(glow_surf, smooth=False), (int(spot["x"]) - r, int(spot["y"]) - r))
        _caption(screen, self.PROMPT if self.result is None else self.feedback,
                 None if self.result is None else self.feedback_color)
        if self.result is None:
...
class WeatherWind:
    ...
    def draw(self, screen):
        ...
        for d in self.debris:
            ...
            x, y = int(d["x"]), int(d["y"])
            if d["type"] == "leaf":
                # 나뭇잎 — 매끈한 타원 대신 픽셀 블록 3단 (곡선 없음)
                pygame.draw.rect(screen, (120, 170, 80), (x - 8, y - 3, 16, 6))
                pygame.draw.rect(screen, (150, 194, 104), (x - 5, y - 5, 8, 2))
                pygame.draw.rect(screen, (86, 132, 60), (x - 5, y + 3, 8, 2))
            else:
                pixel_disc(screen, (140, 130, 120), (x, y), 6, px=2)
                pixel_disc(screen, (170, 160, 150), (x - 1, y - 1), 3, px=2)
        for p in self.puffs:
            r = int(10 * (1 - p[2] / 0.3)) + 3
            s = pygame.Surface((r * 2, r * 2), pygame.SRCALPHA)
            pygame.draw.circle(s, (200, 210, 180, int(160 * (p[2] / 0.3))), (r, r), r)
            screen.blit(pixelate(s, smooth=False), (int(p[0]) - r, int(p[1]) - r))
        screen.set_clip(old_clip)
        _caption(screen, self.PROMPT if self.result is None else self.feedback,
                 None if self.result is None else self.feedback_color)
        if self.result is None:
            _counter(screen, i18n.tf("막은 횟수 {c}/{t}", c=self.blocked, t=self.target),
                     self.timer / 8.0)
