...
- [ ] **흙 보이게(무논)**: 벼 논이 물로 꽉 차 흙이 안 보임. 모 주변에 진흙 둔덕/가장자리 흙 노출.
- [ ] **벼 배수/물고임 메시지**: 관개농업은 물 고임이 정상인데 "물이 고여 있다" 경고가 뜸. 벼는
  물고임 경고 억제하고, 배수 상승 시 '물꼬 트기'(=배수로 정리 라벨)로 조절되게 안내 정리. (farm.py build_notice/메시지)
- [ ] **호미 녹슨 실제로**: rub QTE(아버지의 낡은 호미)가 갈색 원. 실제 호미(날+자루)에 녹슨 자국으로. (story_choice.py rub)
- [ ] **돌 다양하게**: '쓰러진 허수아비' tap QTE 돌이 너무 둥글둥글. 모양·크기·색 다양하게. (story_choice.py tap)
- [ ] **이벤트 다시하기(별잇기 등 재생)**: 갤러리 이야기/기억 모달 '닫기' 옆에 '다시 하기' 버튼 →
  그 이벤트/미니게임 재실행(특히 별잇기). s0t 강력 요청.
- [ ] **가운데 정렬**: 업적 홀수 마지막 칸 단독일 때 가운데로(현재 10종이라 짝수→해소됐을 수 있음, 확인).
- [ ] **이스터에그 추가**: '람쥐썬더' 등 소소한 것. (여유 있을 때)
- [ ] **마지막 사용자명 노출**: 어딘가 이전 사용자명이 뜬다는 제보(위치 미확인). 재현 후 수정.
