...
- **2026-07-07** (피드백 배치 7 반영 완료): 호미 곡선날 전통 모양으로 정밀 재디자인, 물뿌리개 각도 최대 -60도로 더 기울임, 사과 아이콘 하단 음영 삐져나감(오버플로우) 버그 수정, 첫 회차 이름 입력창 '돌아가기' 시 타이틀 복귀 버그 수정, 이스터에그 '서태양' 이름 재기동 시 자동 로드 유지, 악몽 BGM 코드 진행 16화음(~35초)으로 2배 확장.
