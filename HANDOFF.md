...
- [ ] **호미가 아직 호미 같지 않음(#4 재차)**: story_choice `_draw_hoe` 재디자인. 진짜 한국 호미
  (ㄱ자로 꺾인 자루 + 뾰족한 삼각 날, 흙 파는 각도). 참고 이미지 필요할 수도.
- [ ] **벼 물 주는 UI(물꼬) 개구림(#3)**: tending.py WaterPour rice 분기(목재 물꼬+물기둥)를 픽셀 느낌
  나게 리워크. 나무 수문·문턱·쏟아지는 물을 픽셀 스프라이트 톤으로.
