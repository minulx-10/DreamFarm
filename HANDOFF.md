...
- **2026-07-09** (v2.1.1-dev.2 프리릴리스 배포):
  - **QTE 미니게임 오작동 수정**: 선택형 이벤트의 QTE 미니게임(돌 줍기 등)에서 목표 완료 확인 조건식이 `hold` 유형의 분기문 내부에 갇혀 있어 다른 QTE(tap, trail, rub 등) 완료 시 정산 처리가 되지 않던 치명적 버그 수정.
  - **개발자 모드 오류 수정**: F9 개발자 치트 창에서 "수확가능" 및 "성장 +5" 클릭 시 `FarmScene`에 직접 필드가 없어 발생하던 `AttributeError` 크래시를 `farm.sim`을 참조하도록 수정.

- **2026-07-09** (v2.1.1-dev.1 프리릴리스 배포):
  - **세이브 경로 안전화**: 스팀 출시 시 `C:\Program Files` 하위 설치에 대비하여 윈도우 `AppData\Roaming\MongjungNongwon` 경로에 세이브가 저장되도록 `save_system.py` 수정.
  - **기동 속도 최적화**: `main.py` 내 모든 씬 기동 시 일괄 로드에서 필요할 때 인스턴스화하는 지연 초기화(Lazy Loading) 구조로 최적화.
  - **main.py 리팩토링**: 중복되는 이벤트 역배율 마우스 좌표 보정을 `to_virtual_pos`로 통합 및 창 아이콘 크롭 로직 함수화.
  - **안내 및 라이선스 준수**: `README.md` 해상도 불일치 설명 수정, 폰트 SIL OFL 1.1 준수를 위해 루트 `LICENSE` 파일 생성 및 `ending.py` 크레딧 화면에 라이선스 공지 탑재.

- **2026-07-08** (v2.1.1-dev.0 프리릴리스 배포):
  - **버그 수정**:
    - `story_choice.py` 내 `printed_text` 참조 에러 크래시 해결.
    - 이해도 최고 단계명 `"한 조각 of 시간"` -> `"한 조각의 시간"` 오타 수정.
    - 진엔딩 판정 조건 판정 완화(이해도 >= 40, 인내 >= 1, 공감 >= 1, 체력 >= 55, 실수 < 5)로 과도하게 빡빡하던 밸런스 개선.
    - 클리어 직후 세이브 데이터(`save_slot.json`) 자동 소멸 처리로 중복 엔딩 획득 버그 차단.
    - 일지 및 시뮬레이터 알림 내 하드코딩된 "당근" 명칭을 기르는 작물(감자 등) 및 밭/논 구분에 맞춰 동적 치환.
    - 1회 이상 클리어한 경우 엔딩 크레딧 화면에서 키보드 `1`, `2`, `3` 키를 눌러 진/노멀/배드 엔딩 재감상하는 단축키 기능 복원.
  - **버전 변경**: 프로젝트 버전을 `v2.1.1`로 변경.
